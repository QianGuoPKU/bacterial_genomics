...
diamond blastp --db  /data2/qguo/reference_databases/cog/cog.dmnd --ungapped-score 95 --min-score 95 --query $dir_genepredicted_infile/$name/$name'.faa' --out $dir_geneannotated_outfile/$name'_best.tab' -p 40 --query-cover 80 --subject-cover 80 --outfmt 6  --max-target-seqs 1 --evalue 1e-3  --id 80
done
